const en_US = {
  //
  "logs list": "Logs List",
  "log sheets": "Log Sheets",
  "sheet records": "Sheet Records",
  "sheet media": "Sheet Media",
  //
  "add log": "Add New Log",
  "add sheet": "Add New Log Sheet",
  "add record": "Add New Record",
  "add media": "Add Media",
  "edit log": "Edit Log",
  "edit sheet": "Edit Sheet",
  "edit record": "Edit Record",
  //
  add: "Add",
  name: "Name",
  desc: "Description",
  amount: "Amount",
  save: "Save",
  "record date": "Record Date",
  "record time": "Record Time",
  "log date": "Log Date",
  //
  "logs empty":
    "It seems like you don't have any logs. Would you like to make one?",
  "records empty":
    "It seems like you don't have any records. Would you like to make one?",
  "media empty":
    "It seems like you don't have any media. Would you like to make one?",
  //
  "no logs selected": "You have not selected a log. Please select one from",
  "no sheets selected": "You have not selected a log. Please select one from",
  //
  "default log sheet": "Default log sheet",
  "default log sheet desc": "Click on this sheet to start adding records!",
};

export default en_US;
